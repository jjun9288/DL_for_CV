...
def compute_distances_two_loops(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    x_train = x_train.flatten().reshape(num_train, -1)
    x_test = x_test.flatten().reshape(num_test, -1)
...
def compute_distances_one_loop(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    for i in range(num_train):
        dists[i] = torch.sqrt(torch.sum(torch.square((x_train[i] - x_test)), dim=1))
...
def compute_distances_no_loops(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    sx = torch.square(x_train).sum(dim=-1, keepdims=True)  #(num_train, 1)
    sy = torch.square(x_test).sum(dim=-1)  #(num_test)
    xy = torch.matmul(x_train, x_test.T)
    dists = torch.sqrt(sx + sy - 2*xy)
...
def predict_labels(dists: torch.Tensor, y_train: torch.Tensor, k: int = 1):
    ...
    val, idx = torch.topk(dists, k, dim=0, largest=False)  # 각 test set에 대해 제일 가까운 k개의 label 할당
    for i in range(len(y_pred)):
        nearest = y_train[idx[:,i]]
        n_val, n_idx = torch.mode(nearest)  # mode returns the most frequent values and 
        y_pred[i] = n_val
...
def knn_cross_validate(
    ...
    x_chunks, y_chunks = x_train.chunk(num_folds, dim=0), y_train.chunk(num_folds)
    ...
    for i in range(num_folds):
        ...
        x_tr = torch.cat([x_train_folds[j] for j in range(num_folds) if j!=i], dim=0)
        y_tr = torch.cat([y_train_folds[j] for j in range(num_folds) if j!= i], dim=0)
        for k in k_choices:
            if k not in k_to_accuracies:
                k_to_accuracies[k] = []
            classifier = KnnClassifier(x_tr, y_tr)
            accuracy = classifier.check_accuracy(x_val, y_val, k=k)
            #print(accuracy)
...
def knn_get_best_k(k_to_accuracies: Dict[int, List]):
    ...
    for k in k_to_accuracies.keys():
        max_acc.append(max(k_to_accuracies[k]))
    max_acc = torch.tensor(max_acc)
    max_idx = torch.argmax(max_acc)
