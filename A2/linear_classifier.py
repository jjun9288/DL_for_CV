...
def svm_loss_naive(W, X, y, reg):
  ...
  Inputs:
  - W: A PyTorch tensor of shape (D, C) containing weights. 3073 x 10
...
def svm_loss_vectorized(W, X, y, reg):
  ...
  num_train = X.shape[0]
  scores = torch.matmul(X,W)    # 10000 x 10
  ...
  tmp = margin
  tmp[margin > 0] = 1
  row_sum = torch.sum(tmp, dim=1)
  tmp[torch.arange(num_train), y] = -row_sum
  dW = (torch.matmul(X.T, tmp) / num_train) + (2 * reg * W)
...
def predict_linear_classifier(W, X):
  ...
  y_pred = torch.matmul(X,W).argmax(dim=-1)
...
def softmax_loss_naive(W, X, y, reg):
  ...
  for i in range(num_train):
    ...
    softmax = scores / torch.sum(scores)
...
def softmax_loss_vectorized(W, X, y, reg):
  ...
  scores = torch.exp(scores)
  softmax = scores / torch.sum(scores, dim=1, keepdims=True)
  loss = (-torch.sum(torch.log(softmax[torch.arange(num_train), y])) / num_train) + (reg * torch.sum(W*W))
...
def softmax_get_search_params():
  ...
  learning_rates = [1e-5, 1e-2, 1e3]
  regularization_strengths = [1e-5, 1e-4, 1e3]
