...
def three_layer_convnet(x, params):
  ...
  out1 = F.conv2d(x, conv_w1, padding=2) + conv_b1.view(1,len(conv_b1),1,1)
  out1 = F.relu(out1)
  out2 = F.conv2d(out1, conv_w2, padding=1) + conv_b2.view(1,len(conv_b2),1,1)
  out2 = F.relu(out2)
  out2 = flatten(out2)
  scores = F.linear(out2, fc_w, fc_b)
...
def initialize_three_layer_conv_part2(dtype=torch.float, device='cpu'):
  ...
  conv_w1 = nn.init.kaiming_normal_(torch.empty((channel_1, C, kernel_size_1, kernel_size_1), device='cuda'))
  conv_w1.requires_grad=True
  conv_b1 = nn.init.zeros_(torch.empty(channel_1, device='cuda'))
  conv_b1.requires_grad=True
  conv_w2 = nn.init.kaiming_normal_(torch.empty((channel_2, channel_1, kernel_size_2, kernel_size_2), device='cuda'))
  conv_w2.requires_grad=True
  conv_b2 = nn.init.zeros_(torch.empty(channel_2, device='cuda'))
  conv_b2.requires_grad=True
  fc_w = nn.init.kaiming_normal_(torch.empty((num_classes, channel_2 * H * W), device='cuda'))
  fc_w.requires_grad=True
  fc_b = nn.init.zeros_(torch.empty(num_classes, device='cuda'))
  ...
  def __init__(self, Cin, Cout, downsample=False):
    ...
    if not downsample:
        if Cin == Cout:
            ...
            self.shortcut = nn.Conv2d(Cin, Cout, kernel_size=1, padding=0)
    else:
        self.shortcut = nn.Conv2d(Cin, Cout, kernel_size=1, padding=0, stride=2)
